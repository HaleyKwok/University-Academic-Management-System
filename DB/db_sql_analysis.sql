...
-- Query 4:
-- This query checked the number of student and courses by department. It helps us better understand the distribution of students across different departments and improve the resource allocation and curriculum planning. In other words, we can see if more courses should be opened for a given department considering the number of students.
SELECT 
    Student.dept_name,
    COUNT(Student.ID) AS student_count,
    COUNT(Course.course_ID) AS course_count
FROM 
    Student
JOIN 
    Course ON Student.dept_name = Course.dept_name
GROUP BY 
    Student.dept_name
ORDER BY 
    student_count DESC;
...
-- Query 5:
--  This query group by department and return the number of instructors, students, and the student-to-instructor ratio for each department. We try to analyze each department's teaching resources and check the workload and resource allocation within each department.
SELECT 
    Department.dept_name,
    COUNT(Instructor.ID) AS instructor_count,
    COUNT(Student.ID) AS student_count,
    COUNT(Student.ID) / COUNT(Instructor.ID) AS student_instructor_ratio
FROM 
    Department
LEFT JOIN 
    Instructor ON Department.dept_name = Instructor.dept_name
LEFT JOIN 
    Student ON Department.dept_name = Student.dept_name
GROUP BY 
    Department.dept_name
ORDER BY 
    student_count DESC;
...
-- Query 6:
-- this query calculate the time for each course
SELECT
    Section.course_ID,
    Section.sec_id,
    Time_Slot.time_slot_ID,
    TIMESTAMPDIFF(HOUR, CONCAT(Section.semester, ' ', Section.year, ' ', Time_Slot.start_hr, ':', Time_Slot.start_min), CONCAT(Section.semester, ' ', Section.year, ' ', Time_Slot.end_hr, ':', Time_Slot.end_min)) AS duration_hours
FROM
    Section
JOIN
...
-- Query 7:
-- this query calculate the teaching time of each professor and the salary they receive. This SQL query try to match the salary of instructors with the duration of courses they teach, focusing on the teaching aspect rather than research.
SELECT 
    Instructor.name,
    Instructor.salary,
    SUM(TIMESTAMPDIFF(HOUR, CONCAT(Section.semester, ' ', Section.year, ' ', Time_Slot.start_hr, ':', Time_Slot.start_min), CONCAT(Section.semester, ' ', Section.year, ' ', Time_Slot.end_hr, ':', Time_Slot.end_min))) AS total_duration_hours
FROM 
    Instructor
JOIN 
...
JOIN 
    Time_Slot ON Section.time_slot_id = Time_Slot.time_slot_ID
GROUP BY 
    Instructor.name, Instructor.salary;
